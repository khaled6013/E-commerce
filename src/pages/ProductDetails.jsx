...
const ProductDetails = () => {
  ...
  let clientRating = Array.from({ length: 5 }, (_, index) => {
    ...
    return (
      singleProducts.rating > index + 1 ? (<MdOutlineStarPurple500 key={index} />) : singleProducts.rating > number ? <MdOutlineStarHalf key={index} /> : (<MdOutlineStarOutline key={index} />)
    );
  })

  let MainPrice = (singleProducts.price / 100) * (singleProducts.discountPercentage || 0);
      ...
      <div className="lg:py-[80px] py-[40px]">
        <div className="lg:w-10/12 w-11/12 mx-auto">
          <div className="lg:w-[50%]">
            <div className="">
              <img src={singleProducts.thumbnail} alt="" className="lg:w-[400px] w-full" />
            ...
            <div className="flex items-center mt-7 gap-x-3 border-b-1 pb-7 border-[#F0F0F0]">
              <div className="">
                <button className="text-[#262626] font-dm font-bold lg:text-[16px] text-[14px] hover:text-[#FFFFFF] hover:bg-black border-1 border-black lg:py-[16px] py-[10px] lg:px-[40px] px-[28px] cursor-pointer">Add to Wish List</button>
              </div>
              <div className="">
                <button onClick={() => handleCart(singleProducts)} className="text-[#262626] font-dm font-bold lg:text-[16px] text-[14px] hover:text-[#FFFFFF] hover:bg-black border-1 border-black lg:py-[16px] py-[10px] lg:px-[40px] px-[20px] cursor-pointer">Add to Cart</button>
              </div>
            </div>
            <div className=" mt-5  border-b-1 pb-5 border-[#F0F0F0]">
              ...
              <div className={`pt-4 ${show ? "block" : "hidden"}`}>
            ...
            <div className=" mt-5  border-b-1 pb-5 border-[#F0F0F0]">
              ...
              <div className={`pt-4 ${showOne ? "block" : "hidden"}`}>
                <p className="font-dm text-[#767676] text-[16px] font-normal">Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.</p>
              </div>
            </div>
          </div>
          ...
          <div className="mt-5">
            ...
            <div className="p-2">
              {activeTab === "description" && (
                <div className="mt-5 font-dm text-[#767676]">{singleProducts.description}</div>
              )}
              {activeTab === "reviews" && (
                <div className="mt-5">
                      ...
                      <div key={index} className="mb-5">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-x-5">
                            <p className="text-[16px] font-dm text-[#262626]">{item.reviewerName}</p>
                            <p className="flex gap-x-1 text-amber-300">{commandRating}</p>
                          </div>
                          <p className="text-[#767676] text-[16px] font-dm font-normal">
